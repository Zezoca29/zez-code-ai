...
class LLMIntegration {
     ...
     */
    async generateCompleteTest(context, partialTestCode) {
        try {
            // Generate the prompt
     ...
     */
    static generatePrompt(context, partialTestCode) {
...
The test class should be production-ready and demonstrate best practices in Java unit testing.`;
     ...
     */
    buildCompletionPrompt(context, partialTestCode) {
    ...
    extractJavaCode(response) {
        ...
        const javaLines = response
            .split('\n')
            .filter(line => line.includes('public class') ||
            line.includes('@Test') ||
     ...
     */
    validateGeneratedCode(testCode, context) {
        const errors = [];
        const suggestions = [];
        let isValid = true;
     ...
     */
    calculateConfidence(testCode, validation) {
        let confidence = 100;
        ...
        if (testCode.includes('@DisplayName'))
            confidence += 5;
        if (testCode.includes('@Nested'))
            confidence += 5;
        if (testCode.includes('assertThrows'))
            confidence += 10;
     ...
     */
    generateExplanation(validation) {
