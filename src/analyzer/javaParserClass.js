...
function extractFullClassName(code, className) {
...
function findMethodInClass(classInfo, normalizedMethodName) {
    ...
    // Busca case-insensitive
    found = methods.find(m => normalizeIdentifier(m.name).toLowerCase() === normalizedMethodName.toLowerCase());
    if (found) {
        return { found: true, method: found, allMethods: methods.map(m => m.name) };
    }
...
async function parseMethodDetails(methodInfo, classInfo, fullCode) {
