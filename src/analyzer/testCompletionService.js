...
exports.TestCompletionService = void 0;
const contextEngineer_1 = require("./contextEngineer");
const llmIntegration_1 = require("./llmIntegration");
const testValidator_1 = require("./testValidator");
class TestCompletionService {
    llmIntegration;
    constructor(llmConfig) {
        const config = llmConfig || llmIntegration_1.LLMIntegration.createDefaultConfig();
        this.llmIntegration = new llmIntegration_1.LLMIntegration(config);
    }
    /**
     * Completes a partial unit test using LLM
     */
    async completeUnitTest(request) {
        try {
            // Step 1: Extract context from the code
            const context = contextEngineer_1.ContextEngineer.extractCodeContext(request.code, request.className, request.methodName, request.testRequirements);
            // Step 2: Validate the original test code
            const originalValidation = testValidator_1.TestValidator.validateGeneratedTest(request.partialTestCode, {
            ...
            const llmResponse = await this.llmIntegration.generateCompleteTest(context, request.partialTestCode);
            // Step 4: Validate the completed test code
            const completedValidation = testValidator_1.TestValidator.validateGeneratedTest(llmResponse.testCode, {
                className: request.className,
                methodName: request.methodName,
                returnType: context.returnType,
                parameters: context.parameters,
            ...
            const improvements = this.generateImprovementSummary(originalValidation, completedValidation, llmResponse);
            return {
     ...
     */
    generateImprovementSummary(originalValidation, completedValidation, llmResponse) {
        const improvements = [];
        // Score improvement
     ...
     */
    static createCompletionRequest(code, className, methodName, partialTestCode, testLevel) {
     ...
     */
    static generateLLMPrompt(request) {
        // Extract context from the code
        const context = contextEngineer_1.ContextEngineer.extractCodeContext(request.code, request.className, request.methodName, request.testRequirements);
        // Generate the prompt that would be sent to the LLM
        const prompt = llmIntegration_1.LLMIntegration.generatePrompt(context, request.partialTestCode);
        return prompt;
    }
    /**
     * Formats the completion result for display
     */
    static formatCompletionResult(result) {
        let output = '';
        output += '='.repeat(80) + '\n';
...
exports.TestCompletionService = TestCompletionService;
//# sourceMappingURL=testCompletionService.js.map