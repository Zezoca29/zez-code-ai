...
class TestValidator {
     ...
     */
    static validateGeneratedTest(testContent, context) {
        const errors = [];
        const warnings = [];
        const suggestions = [];
     ...
     */
    static checkCriticalErrors(testContent, context, errors) {
     ...
     */
    static checkCommonIssues(testContent, context, warnings) {
     ...
     */
    static checkBestPractices(testContent, context, suggestions) {
     ...
     */
    static calculateQualityScore(testContent, errors, warnings, suggestions) {
        let score = 100;
        ...
        if (testContent.includes('@InjectMocks'))
            score += 5;
        if (testContent.includes('assertThat'))
            score += 5;
        if (testContent.includes('verify('))
            score += 5;
        if (testContent.includes('/**'))
            score += 5;
        if (testContent.includes('// Arrange') && testContent.includes('// Act') && testContent.includes('// Assert'))
            score += 10;
     ...
     */
    static getFixesForIssues(validationResult) {
        const fixes = [];
     ...
     */
    static validateTestScenarios(scenarios, context) {
        const errors = [];
        const warnings = [];
        const suggestions = [];
