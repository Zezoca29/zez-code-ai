...
async function parseJavaFunction(code, functionName) {
...
function findFunctionByLineSearch(lines, functionName) {
...
function extractFunctionDetails(functionInfo, code) {
...
function extractVisibility(signature) {
    ...
    if (signature.includes('private'))
        return 'private';
    if (signature.includes('protected'))
        return 'protected';
    return 'default';
}
function extractReturnType(signature, body) {
...
function extractAnnotations(code, startLine) {
...
function extractThrowsExceptions(signature) {
...
function extractLocalVariables(body) {
    const variables = [];
...
function extractCommonMethodCalls(body) {
    const functions = [];
...
function removeDuplicateCalls(functions) {
    const seen = new Set();
    const unique = [];
...
function extractMethodCallParameters(paramString) {
...
function smartSplitParameters(str) {
    const result = [];
    let current = '';
    let depth = 0;
    let inString = false;
...
function calculateBasicComplexity(body) {
...
function createAnalysisFromLine(line, functionName, totalLines) {
...
function createSmartDefaultAnalysis(code, functionName) {
    ...
    if (hasIntParams) {
        parameters.push({ name: 'a', type: 'int', annotations: [] }, { name: 'b', type: 'int', annotations: [] });
    }
    return {
        name: functionName,
