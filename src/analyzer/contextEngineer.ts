...
export interface TestRequirements {
  testLevel: 'basic' | 'intermediate' | 'advanced' | 'comprehensive';
  focusAreas: string[];
  mockStrategy: 'minimal' | 'comprehensive' | 'realistic';
  assertionStyle: 'basic' | 'detailed' | 'business-focused';
  testDataStyle: 'simple' | 'realistic' | 'edge-cases';
}
...
export class ContextEngineer {
   ...
   */
  static generateLLMPrompt(context: CodeContext): LLMPrompt {
...
- Add comments for complex test logic`;
   ...
   */
  private static generateUserPrompt(context: CodeContext): string {
...
8. Meaningful test documentation`;
   ...
   */
  private static generateExamples(context: CodeContext): string[] {
...
public void testMethodThrowsException() {
   ...
   */
  private static generateConstraints(context: CodeContext): string[] {
   ...
   */
  private static parseMethodInfo(code: string, className: string, methodName: string): any {
    // This is a simplified parser - in a real implementation, you'd use a proper Java parser
    const methodRegex = new RegExp(
      `(public|private|protected)?\\s*(static\\s+)?(\\w+)\\s+${methodName}\\s*\\([^)]*\\)\\s*\\{[\\s\\S]*?\\}`,
      'g'
    );
   ...
   */
  private static extractBusinessLogic(methodBody: string): string[] {
    const logic: string[] = [];
   ...
   */
  private static calculateComplexity(methodBody: string): number {
   ...
   */
  private static extractMethodSignature(methodBody: string): string {
  ...
  private static extractParameters(signature: string): Array<{name: string, type: string}> {
  ...
  private static extractCalledMethods(methodBody: string): Array<{className: string, methodName: string, parameters: string[], returnType: string, isStatic: boolean}> {
    // Simplified extraction - in reality, you'd use a proper Java parser
    const methodCalls: Array<{className: string, methodName: string, parameters: string[], returnType: string, isStatic: boolean}> = [];
  ...
  private static extractAnnotations(methodBody: string): string[] {
  ...
  private static extractThrowsExceptions(methodBody: string): string[] {
