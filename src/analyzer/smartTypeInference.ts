...
export interface TypeMapping {
  ...
  validValues: string[];
  imports: string[];
  edgeCases: Array<{ name: string; value: string; description: string }>;
}

export class SmartTypeInference {
  
  private static readonly TYPE_MAPPINGS: { [key: string]: TypeMapping } = {
    // Primitive types
    'int': {
      javaType: 'int',
      defaultValue: '1',
   ...
   */
  static inferType(type: string, parameterName?: string): TypeInferenceResult {
   ...
   */
  static convertToJavaValue(value: any, targetType: string): TypeInferenceResult {
   ...
   */
  static generateMockReturnValue(returnType: string): string {
    ...
    switch (inference.javaType) {
      ...
      case 'long': case 'Long': return '1L';
      case 'double': case 'Double': return '1.0';
      case 'float': case 'Float': return '1.0f';
      case 'boolean': case 'Boolean': return 'true';
      case 'char': case 'Character': return "'m'";
      case 'List': case 'List<Object>': return 'new ArrayList<>()';
      case 'Set': case 'Set<Object>': return 'new HashSet<>()';
      case 'Map': case 'Map<String, Object>': return 'new HashMap<>()';
   ...
   */
  private static cleanTypeString(type: string): string {
   ...
   */
  private static inferGenericType(genericType: string): TypeInferenceResult {
   ...
   */
  private static inferCustomClass(className: string, parameterName?: string): TypeInferenceResult {
   ...
   */
  private static inferFromParameterName(parameterName: string): TypeInferenceResult | null {
   ...
   */
  private static inferTypeFromArray(array: any[]): string {
    if (array.length === 0) return 'Object';
    
    const firstElement = array[0];
    if (typeof firstElement === 'string') return 'String';
    if (typeof firstElement === 'number') return Number.isInteger(firstElement) ? 'Integer' : 'Double';
    if (typeof firstElement === 'boolean') return 'Boolean';
